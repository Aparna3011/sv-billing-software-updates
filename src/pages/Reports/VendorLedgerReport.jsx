...
export default function VendorLedgerReport() {
  ...
  async function load(nextFilters = filters) {
    ...
    setLedger(await modules.reports.vendorLedger(nextFilters));
  }

  useEffect(() => {
    modules.vendors.list().then(setVendors).catch((error) => toast.error(error.message));
  }, []);
  ...
  return (
    <ContentArea>
      <PageHeader
        title="Vendor Ledger"
        subtitle="Vendor purchase bills, payments, and payable running balance"
        actions={<ReportExportButtons title="Vendor Ledger" columns={exportColumns} rows={exportRows} />}
      />
      ...
      <div className="mt-6">
        <DataTable
          ...
          columns={[
            ...
            { accessorKey: "balance", header: "Balance", cell: (info) => money(info.getValue()) },
          ]}
        />
      </div>
    </ContentArea>
