...
const getInvoiceItemRows = (invoice = {}) => {
  ...
  const subtotal =
    invoice.subtotal != null
      ? roundAmt(invoice.subtotal)
      : roundAmt(bases.reduce((sum, base) => sum + base, 0));
  const discountIsPercent = isPercentDiscount(invoice.discount_is_percent);
  ...
  return items.map((item, index) => {
    ...
    const savedCgst = roundAmt(item.cgst);
    const savedSgst = roundAmt(item.sgst);
    const savedIgst = roundAmt(item.igst);
    const hasSavedTax = savedCgst > 0 || savedSgst > 0 || savedIgst > 0;
    ...
    if (
      ...
      const customerState = normalizeState(invoice?.state);
      const companyState = normalizeState(invoice?.company_state);
...
export default function InvoiceDetail() {
  ...
  async function printPdf() {
    ...
    try {
      await modules.pdf.invoice(Number(id), "print");
  ...
  async function exportPdf() {
    ...
    try {
      await modules.pdf.invoice(Number(id), "export");
  ...
  function shareEmail() {
    ...
    const body = encodeURIComponent(
      `Dear ${invoice?.company_name || "Client"},\n\nPlease find the invoice details attached/exported from SV IT Hub Billing.\n\nRegards`,
    );
    window.location.href = `mailto:${invoice?.email || ""}?subject=${subject}&body=${body}`;
  }
  function shareWhatsApp() {
    const text = encodeURIComponent(
      `Invoice ${invoice?.invoice_no} for ${invoice?.company_name}: ${money(invoice?.grand_total)}. Balance due: ${money(invoice?.balance_due)}.`,
    );
    window.open(`https://wa.me/?text=${text}`, "_blank");
  }
  const invoiceStatus = String(invoice?.status || "").toLowerCase();
  const isPaid =
    invoiceStatus === "paid" || Number(invoice?.balance_due || 0) <= 0;
  const itemRows = getInvoiceItemRows(invoice || {});
  ...
  const itemTotals = itemRows.reduce(
    ...
    { discountedAmount: 0, cgst: 0, sgst: 0, igst: 0, total: 0 },
  );
  const displayTaxTotal = roundAmt(
    itemTotals.cgst + itemTotals.sgst + itemTotals.igst,
  );
  const displayRoundOff = roundAmt(
    Number(invoice?.grand_total || 0) - itemTotals.total,
  );
    ...
    <ContentArea>
      ...
      <div className="mb-4 grid grid-cols-5 gap-4 rounded-lg border bg-white p-4 text-sm">
        ...
        <div>
          Date
          <br />
          <strong>{date(invoice?.invoice_date)}</strong>
        </div>
        <div>
          Total
          <br />
          <strong>
            ₹
            {Math.round(Number(invoice?.grand_total || 0)).toLocaleString(
              "en-IN",
            )}
          </strong>
        </div>
        <div>
          ...
          <strong>
            ₹
            {Math.round(Number(invoice?.paid_amount || 0)).toLocaleString(
              "en-IN",
            )}
          </strong>
        </div>
        <div>
          ...
          <strong>
            ₹
            {Math.round(Number(invoice?.balance_due || 0)).toLocaleString(
              "en-IN",
            )}
          </strong>
        </div>
            ...
            <thead className="bg-slate-50 text-xs uppercase text-slate-600">
              <tr>
                ...
                <th className="px-4 py-3 font-semibold">
                  {isPercentDiscount(invoice?.discount_is_percent)
                    ? "Discount (%)"
                    : "Discount (₹)"}
                </th>
              ...
              {itemRows.length > 0 && (
                <tr className="bg-slate-100 font-semibold">
                  ...
                  <td className="px-4 py-4 text-teal-700">
                    {money(invoice?.grand_total ?? itemTotals.total)}
                  </td>
                </tr>
              )}
            </tbody>
        ...
        {viewPayment && (
          <div className="grid gap-4">
            ...
            <div className="rounded-lg border p-4">
              ...
              <div className="text-base font-medium">
                ₹
                {Math.round(Number(viewPayment.amount || 0)).toLocaleString(
                  "en-IN",
                )}
              </div>
            </div>
