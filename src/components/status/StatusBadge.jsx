const STATUS_CONFIG = {
  draft: { label: 'Draft', color: 'bg-slate-100 text-slate-700' }, // Updated to gray badge
  ...
  accepted: { label: 'Accepted', color: 'bg-emerald-100 text-emerald-700' }, // Renamed from 'approved'
  declined: { label: 'Declined', color: 'bg-red-100 text-red-700' }, // Renamed from 'rejected'
  ...
  // Removed 'approved' and 'rejected' as per standardization
};

export default function StatusBadge({ status }) {
  const normalizedStatus = String(status || '').toLowerCase();
  const config = STATUS_CONFIG[normalizedStatus] || { label: status || 'Unknown', color: 'bg-slate-100 text-slate-600' };
